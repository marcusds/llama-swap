...
models:
  ...
  # memoryBudget priority example:
  "priority-model":
    # priority: eviction priority for the memoryBudget router
    # - optional, default: 0
    # - only used when the memoryBudget router is active
    # - higher priority models are evicted last, ties broken by
    #   least-recently-used
    # - an entry for this model under memoryBudget.models wins over this field
    priority: 10
...
#
# Priorities can be set two ways, and both can be mixed in one config:
#   1. In the memoryBudget.models block below.
#   2. As a `priority:` field on the model itself, in the models section.
#      Handy when priorities are easier to maintain next to the model they
#      belong to.
# If a model appears in both places the memoryBudget.models entry wins.
# Models with no priority in either place default to priority: 0.
#
